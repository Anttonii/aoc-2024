...
vector<int> findCheats(vector<string> &input, vector<vector<int>> &costs)
{
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            ...
            {
                for (auto d : get_dirs(input, pair(i, j), 2))
                {
                    pair<int, int> newPos = utils::add_pairs(pair(i, j), d);
                    int save = costs[newPos.first][newPos.second] - costs[i][j];
                    if (save > 2)
                        cheats.push_back(save - 2);
                }
            }
        }
    }
